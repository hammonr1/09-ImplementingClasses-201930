...
class Point(object):
    ...
    def move_to(self,x,y):
        prev_x = self.x
        prev_y = self.y
        prev = Point(prev_x, prev_y)
        self.x = x
        self.y = y

        self.num = self.num +1
        self.sum = self.sum + self.get_distance_from(prev)
    ...
    def get_distance_from_start(self):
        d = math.sqrt((self.initial_x - self.x) ** 2 + (self.initial_y - self.y) ** 2)
        return d
